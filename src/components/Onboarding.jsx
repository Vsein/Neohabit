...
export default function Onboarding() {
  ...
  const datePeriodLength = Math.min(
    mobile
      ? adaptiveDatePeriodLength - (width < 550) * 3 - (width >= 550 && width < 700) * 1
      : adaptiveDatePeriodLength - 3,
    21,
  );
  ...
  return (
    <div className="overlay overlay-active">
      <div className={`modal modal-active modal-onboarding ${mobile ? 'modal-mobile' : ''}`}>
        ...
        {mobile ? <MobileOnboarding slide={slide} /> : <DesktopOnboarding slide={slide} />}
        {mobile ? (
...
function DesktopOnboarding({ slide }) {
  return (
    <>
      {slide === 1 && (
        <>
...
function MobileOnboarding({ slide }) {
  return (
    <>
      {slide === 1 && (
        <>
